...
public class ColorPicker extends View {
    ...
    Integer  mColorGlowdeck = null ;
    public Integer getColorGlowdeck() {
		return mColorGlowdeck;
	}


     public void setColorGlowdeck(Integer colorOnCurrentGlowdeck) {
		this.mColorGlowdeck = colorOnCurrentGlowdeck;
	}
	...
	public OpacityBar getmOpacityBar() {
		return mOpacityBar;
	}
	...
	public ColorPicker(Context context, AttributeSet attrs) {
		...
		//Log.d(TAG,"ColorPicker context   attrs") ;
	}

	public ColorPicker(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		//Log.d(TAG,"ColorPicker context   attrs defStyle") ;
		try{
		init(attrs, defStyle);
		}catch(Exception e){e.printStackTrace();}
	}
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		{
			CurrentGlowdecks currentGlowdecks = mainActivity.getCurrentGlowdecks() ;
			if (currentGlowdecks != null)
			{
				mColorGlowdeck = currentGlowdecks.getColorFromCurrentGlowdeck() ;
			}
		}

		if (StreamsApplication.DEBUG_MODE)
		{
			if (mColorGlowdeck != null)
			{
				if (StreamsApplication.DEBUG_MODE)
				{
			Log.d("dbg","color in current glowdeck = " + Color.alpha(mColorGlowdeck) + ","
					+ Color.red(mColorGlowdeck) + "," + Color.green(mColorGlowdeck)
					+ "," + Color.blue (mColorGlowdeck));
				}
			}
			float[ ] hsvColor = new float[3];
			if (mColorGlowdeck != null)
			{
			Color.colorToHSV(mColorGlowdeck, hsvColor);
			if (StreamsApplication.DEBUG_MODE)
			{
		...
		if (mColorGlowdeck != null)
		{
			mAngle = colorToAngle(mColorGlowdeck) ;
			mCenterNewColor =   mColorGlowdeck ;
			mCenterOldColor  =   mColorGlowdeck ;

			Color.colorToHSV(mColorGlowdeck, mHSV);
		}
		else
		{
			mAngle = (float) (-Math.PI / 2);
			mColor =  calculateColor(mAngle);
			mColorGlowdeck = mColor ;
		}
		...
		mCenterNewPaint.setColor(mColorGlowdeck);
		...
		mCenterOldPaint.setColor(mColorGlowdeck);
	...
	public void setColor(int color) {
		...
		int colorToUse = mColor ;
		//if (SVBar.newCircleInitialized)
		//{
		//	colorToUse = mColorGlowdeck | ALPHA ;
		//}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (mSVbar == null)
		{
			//Log.d(TAG,"onTouchEvent::mSVbar::return") ;
			return false ;
		}
		...
		SVBar.newCircleInitialized = true ;
		...
		case MotionEvent.ACTION_DOWN:
			...
			// If user did not press pointer or center, report event not handled
				...
				if (!PickerFragment.isInitial() || mHSV[1] != 0.0f )
				{

				return false;
				}
			}
			if (PickerFragment.isInitial())
			{
				PickerFragment.setInitial(false) ;
				if (mHSV[1] == 0.0f)
				{
					setColorGlowdeck(mColor) ;
					mSVbar.initColor() ;
		...
		case MotionEvent.ACTION_UP:
			...
			streamsApplication.getBluetoothSppManager().sendMessage(PickerFragment.getCOLchangeString( getColor()));


			CurrentGlowdecks currentGlowdecks = MainActivity.getMainActivity().getCurrentGlowdecks() ;
			if (currentGlowdecks != null)
			{
				currentGlowdecks.updateCurrentGlowdeckColor(PickerFragment.getRGBStringColors( getColor())) ;
				// check to see if Light System is off
	...
	public void addSVBar(SVBar bar) {
		...
		if (PickerFragment.reEntry)
		{
		mSVbar.setColor(mColor);
		}
		else
		{
		mSVbar.setColor(mColorGlowdeck | ALPHA ); 
		}
		}catch(Exception e){e.printStackTrace();}
	}
	...
	public void addOpacityBar(OpacityBar bar) {
		...
		mOpacityBar.setColor(mColorGlowdeck | ALPHA );
		}catch(Exception e){e.printStackTrace();}
	...
	public void addSaturationBar(SaturationBar bar) {
		...
		mSaturationBar.setColor(mColorGlowdeck  | ALPHA  );
		}catch(Exception e){e.printStackTrace();}
	}
	...
	public void addValueBar(ValueBar bar) {
		...
		mValueBar.setColor(mColorGlowdeck | ALPHA  );
		}catch(Exception e){e.printStackTrace();}
	}
